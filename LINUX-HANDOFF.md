...
| L11 Packaging            | **not run** | AppImage and `.deb` install and launch on Ubuntu 24.04 LTS (the glibc floor — see below)   |
...
| Distros older than glibc 2.39 | The release job builds on `ubuntu-24.04`, and a Linux binary will not start on an older glibc than it was built against — so the packages need Ubuntu 24.04+, Debian 13+ or Fedora 40+. Ubuntu 22.04 LTS is supported upstream until 2027 and is excluded. It was the previous pin; that runner image begins deprecation on 2026-09-17, so the durable fix is building inside an older-base container on a supported runner, not re-pinning to a retiring image. |

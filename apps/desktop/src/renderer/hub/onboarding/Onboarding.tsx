...
import { downloadPercent, formatBytes } from '../../lib/format'
  ...
  return (
    <div className="mx-auto flex h-full max-w-2xl flex-col px-10 py-9">
      <header className="mb-6">
        ...
        <div className="mt-2">
          <ProgressBar percent={((index + 1) / steps.length) * 100} label="Onboarding progress" />
        </div>
      </header>
      ...
      <footer className="mt-8 flex items-center justify-between gap-4 border-t border-line pt-5">
        <Button tone="ghost" onClick={back} disabled={index === 0}>
          Back
        </Button>
        <div className="flex items-center gap-2">
          {isSkippable(step) ? (
            <Button tone="ghost" onClick={advance}>
              Skip this step
            </Button>
          ) : null}
          <Button tone="primary" onClick={last ? finish : advance}>
...
function ModelStep({ settings }: { settings: Settings }): React.JSX.Element {
  const { models, downloads, start, error } = useModels()
  ...
  const choose = useCallback(
    (optionId: string) => {
      ...
      for (const entry of option.entries) {
        ...
        if (!installed) start(entry.id)
      }
    },
    [models, options, start],
  )
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 rounded-lg border border-line bg-canvas px-3 py-2">
        ...
        <Button tone="ghost" onClick={simulate} disabled={simulating}>
          Simulate a hotkey
        </Button>
      </div>
    ...
    <div>
      ...
      <Card className="mt-6">
        ...
        <ol className="mt-2 space-y-1.5 text-[13px] leading-relaxed text-ink-muted">
          <li>1. Open Keyboard settings with the button below.</li>
